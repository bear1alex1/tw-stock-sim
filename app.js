...
/* ===== v3.9.1 integrated screener & AI module ===== */
...
const SCREEN_MESSAGES = {
  ...
  generalLimit:'本次候選股票超過一般使用者上限 100 檔。請縮小條件、改用自訂清單，或請管理員解鎖後再執行。',
...
function safeNum(v){ return typeof num === 'function' ? num(v) : Number(v); }
function formatN(v, d){ const n = safeNum(v); return n == null || !isFinite(n) ? '—' : Number(n).toFixed(d == null ? 2 : d); }
function formatP(v){ return typeof formatPrice === 'function' ? formatPrice(v) : formatN(v,2); }
function showToastMsg(msg){ try{ showToast(msg); }catch(_e){ alert(msg); } }
function getScreenMessage(key){ return SCREEN_MESSAGES[key] || '系統忙碌中，請稍後再試。'; }
function getStockMetaSafe(symbol){ try{ return typeof getStockMeta === 'function' ? (getStockMeta(symbol)||{}) : ((typeof stockMetaCache!=='undefined'&&stockMetaCache[symbol])||{}); }catch(_e){ return {}; } }
function stockNameOf(symbol){ try{ return typeof getStockName === 'function' ? (getStockName(symbol)||'') : ''; }catch(_e){ return ''; } }
...
function isNonCompanyInstrument(symbol, name){
  symbol = normalizeSymbol(symbol);
  const meta = getStockMetaSafe(symbol);
  const joined = [symbol, name || '', stockNameOf(symbol), meta.stock_name, meta.name, meta.industry_category, meta.market, meta.type].map(function(v){ return String(v||''); }).join('|');
...
function tone(score){ const n = safeNum(score); return n==null?'':(n>=65?'positive':(n<=35?'negative':'')); }
function signedTone(v){ const n=safeNum(v); return n==null?'':(n>0?'positive':(n<0?'negative':'')); }
function renderToneValue(value, formatted){ const t=signedTone(value); return '<span class="'+(t==='positive'?'screen-price-positive':t==='negative'?'screen-price-negative':'')+'">'+formatted+'</span>'; }
function setScreenPoolHint(){
  ...
  else if(v==='allStocks') hint.textContent=(_screenAdmin.unlocked&&_screenAdmin.fullscan)?'目前將以全部股票代號作為候選池。':'全部股票模式已被鎖定，僅限管理員開啟。';
}
function updateScreenAdminUI(){
  ...
  if(status){
    if(_screenAdmin.unlocked){
      status.textContent='管理模式｜'+(_screenAdmin.fullscan?'全掃已開啟':'全掃未開啟')+'｜'+(_screenAdmin.widerange?'大範圍已開啟':'大範圍未開啟');
  ...
  if(tools) tools.classList.toggle('show', !!_screenAdmin.unlocked);
  if(allOpt){
    const allow = !!(_screenAdmin.unlocked && _screenAdmin.fullscan);
    allOpt.hidden = !allow;
...
function openScreenAdminModal(){
  const now=Date.now();
  if(_screenAdmin.cooldownUntil && now<_screenAdmin.cooldownUntil){ alert(getScreenMessage('adminCooldown')); return; }
...
async function submitScreenAdminUnlock(){
  ...
  try{
    const digest=await sha256Hex(SCREEN_ADMIN_PEPPER+'|'+String(pwd)+'|unlock');
    if(digest===SCREEN_ADMIN_HASH){
      _screenAdmin.unlocked=true; _screenAdmin.fullscan=true; _screenAdmin.widerange=true; _screenAdmin.failed=0;
    ...
    if(_screenAdmin.failed>=3){ _screenAdmin.cooldownUntil = Date.now()+30000; _screenAdmin.failed=0; if(msg) msg.textContent=getScreenMessage('adminCooldown'); return; }
    if(msg) msg.textContent=getScreenMessage('adminBadPwd');
  }catch(e){ if(msg) msg.textContent='驗證失敗：'+(e&&e.message?e.message:e); }
}
function lockScreenAdmin(){ _screenAdmin.unlocked=false; _screenAdmin.fullscan=false; _screenAdmin.widerange=false; updateScreenAdminUI(); alert(getScreenMessage('adminLocked')); }
function handleScreenAdminAction(action){
  if(!_screenAdmin.unlocked){ alert(getScreenMessage('adminOnly')); return; }
  if(action==='fullscan'){ _screenAdmin.fullscan=!_screenAdmin.fullscan; alert(_screenAdmin.fullscan?'管理員已開啟「全部股票」選項。':'已關閉全部股票。'); }
  else if(action==='widerange'){ _screenAdmin.widerange=!_screenAdmin.widerange; alert(_screenAdmin.widerange?'管理員已開啟大範圍掃描。':'已關閉大範圍掃描。'); }
  else if(action==='lockback'){ lockScreenAdmin(); return; }
  updateScreenAdminUI();
}
function getAllStockSymbols(){
  try{
    return Object.keys(stockNameCache).filter(function(code){ return /^\d{4,6}$/.test(code); }).sort();
  }catch(_e){ return []; }
}
async function getScreenerUniverse(criteria){
  let arr=[];
  if(criteria.universe==='watchlist') arr=[...(state.watchlist||[])];
  else if(criteria.universe==='holdings') arr=Object.keys(state.holdings||{});
  else if(criteria.universe==='union') arr=[...(state.watchlist||[]),...Object.keys(state.holdings||{})];
  else if(criteria.universe==='custom') arr=(document.getElementById('screenCustomList')?.value||'').split(/[\s,，;；]+/).map(normalizeSymbol).filter(Boolean);
  else if(criteria.universe==='allStocks'){ await preloadStockNames(); arr=getAllStockSymbols(); }
  arr=[...new Set(arr.map(normalizeSymbol).filter(Boolean))].filter(function(symbol){ return !isNonCompanyInstrument(symbol); });
  return arr;
}
function getScreenerCriteria(){
  return {
    minTotal: safeNum(document.getElementById('screenMinTotal')?.value),
...
function scoreTech(item){
  let s=50;
  if(item.price!=null&&item.ma20!=null){ if(item.price>=item.ma20) s+=12; else s-=12; }
  if(item.rsi!=null){ if(item.rsi<30) s+=10; else if(item.rsi>70) s-=10; else if(item.rsi>=40&&item.rsi<=60) s+=4; }
  return Math.max(0, Math.min(100, Math.round(s)));
}
function scoreFund(fund){
  if(!fund || fund.notApplicable) return null;
  let s=50;
  if(fund.eps!=null) s += fund.eps>0 ? 18 : -18;
  if(fund.pe!=null) s += (fund.pe>0 && fund.pe<25) ? 10 : (fund.pe>=25 ? -6 : 0);
  if(fund.pbr!=null) s += (fund.pbr>0 && fund.pbr<3) ? 8 : (fund.pbr>=5 ? -8 : 0);
  if(fund.dividendYield!=null) s += safeNum(fund.dividendYield)>=2 ? 8 : 0;
  return Math.max(0, Math.min(100, Math.round(s)));
}
function formatScreenCriteria(criteria){
  const map={watchlist:'追蹤清單',holdings:'持股庫存',union:'追蹤+持股',custom:'自訂清單',allStocks:'全部股票'};
  return ['資料池：'+(map[criteria.universe]||criteria.universe),'綜合≥'+criteria.minTotal,'顯示上限 '+criteria.limit+' 檔'];
}
function buildReasonTags(item){
  const tags=[];
  if(item.price!=null&&item.ma20!=null) tags.push({text:item.price>=item.ma20?'站上 MA20':'跌破 MA20', tone:item.price>=item.ma20?'positive':'negative'});
  if(item.rsi!=null) tags.push({text:'RSI '+item.rsi.toFixed(2), tone:item.rsi<30?'positive':item.rsi>70?'negative':'neutral'});
  if(item.fund?.notApplicable) tags.push({text:'基本面不適用', tone:'neutral'});
  else if(item.fundScore!=null) tags.push({text:'基本面分數 '+item.fundScore, tone:tone(item.fundScore)||'neutral'});
  return tags.slice(0,4);
}
function renderScreenerHistory(){
  const box=document.getElementById('screenHistoryList');
  if(!box) return;
  const arr=Array.isArray(state.screenHistory)?state.screenHistory:[];
  if(!arr.length){ box.innerHTML='<div class="muted" style="font-size:.8rem;">尚無篩選紀錄</div>'; return; }
  box.innerHTML=arr.map(function(r){
    return '<div style="padding:10px 0;border-bottom:1px solid var(--border2);font-size:.8rem;line-height:1.7;">'
      +'<div style="color:#fff;font-weight:700;">'+(r.time||'')+'</div>'
      +'<div style="color:#bfd8ff;">'+(r.poolLabel||'')+'｜命中 '+(r.count||0)+' 檔</div>'
      +'<div class="muted">'+((r.filters||[]).join('、'))+'</div></div>';
  }).join('');
}
function poolLabelByValue(v){ return ({watchlist:'追蹤清單',holdings:'持股庫存',union:'追蹤 + 持股',custom:'自訂清單',allStocks:'全部股票'})[v] || v; }
...
function renderScreenResults(rows, criteria, stats){
  ...
  rows.sort(function(a,b){
    ...
    if(sortBy==='priceDesc') return (b.price??-999)-(a.price??-999);
  ...
  summary.innerHTML = '<span class="screen-result-chip">掃描 '+stats.total+' 檔</span><span class="screen-result-chip positive">成功 '+stats.ok+' 檔</span><span class="screen-result-chip negative">失敗 '+stats.failed+' 檔</span>' + formatScreenCriteria(criteria).map(function(t){ return '<span class="screen-result-chip">'+t+'</span>'; }).join('');
  if(!rows.length){ empty.style.display=''; return; }
  empty.style.display='none';
  rows.forEach(function(r){
    const tr=document.createElement('tr');
    const reasons=buildReasonTags(r);
    tr.innerHTML=''
      ...
      +'<td><div class="screen-score-wrap"><span class="screen-score-box '+tone(r.totalScore)+'">'+r.totalScore+'</span></div><div class="screen-score-sub">綜合分數</div></td>'
      +'<td><div class="screen-score-wrap"><span class="screen-score-box '+tone(r.techScore)+'">'+r.techScore+'</span><span class="screen-score-box '+tone(r.fundScore)+'">'+(r.fundScore==null?'N/A':r.fundScore)+'</span></div><div class="screen-score-sub">技術 / 基本面</div></td>'
      +'<td><div class="'+(r.price>=r.ma20?'screen-price-positive':'screen-price-negative')+'">'+formatP(r.price)+'</div><div style="font-size:.72rem;color:#8b949e;">MA20 '+formatP(r.ma20)+'</div></td>'
      +'<td><div>'+(r.rsi==null?'—':r.rsi.toFixed(2))+'</div></td>'
      +'<td><div>'+renderToneValue(r.fund?.eps, r.fund?.notApplicable?'不適用':(r.fund?.eps==null?'—':String(r.fund.eps)))+'</div><div style="font-size:.72rem;color:#8b949e;">殖利率 '+(r.fund?.notApplicable?'不適用':(r.fund?.dividendYield==null?'—':String(r.fund.dividendYield)+'%'))+'</div></td>'
      +'<td><button class="btn-xs btn-xs-blue" data-screen-ai="'+r.symbol+'">AI</button></td>';
    tbody.appendChild(tr);
  });
  document.querySelectorAll('[data-screen-ai]').forEach(function(btn){ btn.onclick=function(){ generateAIReport(btn.dataset.screenAi); }; });
}
function clearScreenerInputs(){
  const defaults={screenMinTotal:'55',screenSortBy:'totalDesc',screenLimit:'20',screenUniverse:'watchlist',screenCustomList:''};
  Object.keys(defaults).forEach(function(id){ const el=document.getElementById(id); if(el) el.value=defaults[id]; });
  document.getElementById('screenResultBody').innerHTML='';
  document.getElementById('screenResultEmpty').style.display='';
  document.getElementById('screenResultSummary').textContent='尚未開始篩選。';
...
async function exportScreenerPdf(){
  ...
  _screenAdmin.pdfBusy=true; setScreenerRunning(_screenScanJob.running);
  try{
    const source=document.getElementById('screenResultCard');
    const wrap=document.createElement('div'); wrap.className='screen-offscreen-pdf';
    const now=new Date();
    wrap.innerHTML='<div style="font-size:22px;font-weight:900;color:#fff;margin-bottom:6px;">台股虛擬操盤系統｜篩選結果報表</div>'
      +'<div style="font-size:12px;color:#9fb4d2;line-height:1.8;">匯出日期：'+now.toLocaleDateString('zh-TW')+'｜匯出時間：'+now.toLocaleTimeString('zh-TW')+'｜版本：v3.9.1</div>'
    ...
    let hLeft=imgH; let posY=12;
    pdf.addImage(img,'PNG',12,posY,imgW,imgH); hLeft-=pageH;
    ...
    wrap.remove();
  }catch(e){ alert('PDF 匯出失敗：'+(e&&e.message?e.message:e)); }
  finally{ _screenAdmin.pdfBusy=false; setScreenerRunning(_screenScanJob.running); }
}
async function analyzeInstrument(symbol){
  const name=stockNameOf(symbol)||symbol;
  const rows=await fetchKLineData(symbol);
  if(!rows || rows.length<30) throw new Error('價格資料不足');
  const closes=rows.map(r=>safeNum(r.close)).filter(v=>v!=null);
  const last=closes[closes.length-1];
  const ma20=avg(closes.slice(-20));
  const rsi=rsi14(closes);
  let fund;
  if(isNonCompanyInstrument(symbol,name)){
    fund={ notApplicable:true, eps:null, dividendYield:null, pe:null, pbr:null };
  }else{
    const f=await fetchFundamentals(symbol);
    fund=f ? { ...f, notApplicable:false } : { eps:null, dividendYield:null, pe:null, pbr:null, notApplicable:true };
  }
  const techScore=scoreTech({price:last,ma20:ma20,rsi:rsi});
  const fundScore=scoreFund(fund);
  const totalScore=Math.round(techScore*0.7 + ((fundScore==null?50:fundScore)*0.3));
  return { symbol, name, price:last, ma20, rsi, fund, techScore, fundScore, totalScore };
}
async function runScreener(){
  const status=document.getElementById('screenRunStatus');
  const summary=document.getElementById('screenResultSummary');
  const empty=document.getElementById('screenResultEmpty');
  if(_screenScanJob.running){ alert(getScreenMessage('pending')); return; }
  const criteria=getScreenerCriteria();
  if(criteria.universe==='allStocks' && !(_screenAdmin.unlocked && _screenAdmin.fullscan)){ alert(getScreenMessage('adminOnly')); return; }
  const symbols=await getScreenerUniverse(criteria);
  if(!symbols.length){
    if(status) status.textContent='沒有可篩選的股票';
    if(summary) summary.textContent='請先建立追蹤清單、持股、自訂清單，或調整條件。';
    document.getElementById('screenResultBody').innerHTML='';
    if(empty) empty.style.display='';
    return;
  }
  if(symbols.length>SCREEN_SCAN_HARD_LIMIT){ alert(getScreenMessage('hardLimit')); return; }
  if(symbols.length>SCREEN_SCAN_SOFT_LIMIT && !(_screenAdmin.unlocked && _screenAdmin.widerange)){ alert(getScreenMessage('generalLimit')); return; }
  if(symbols.length>SCREEN_SCAN_SOFT_LIMIT && (_screenAdmin.unlocked && _screenAdmin.widerange)){
    if(!confirm(getScreenMessage('adminLargeConfirm'))){ if(status) status.textContent='已取消大量掃描'; return; }
  }
  _screenScanJob.running=true; _screenScanJob.cancelled=false; setScreenerRunning(true);
  ...
  document.getElementById('screenResultBody').innerHTML='';
  if(empty) empty.style.display='';
  const rows=[]; let okCount=0, failCount=0;
  try{
    for(let i=0;i<symbols.length;i++){
      if(_screenScanJob.cancelled) break;
      const symbol=normalizeSymbol(symbols[i]);
      if(status) status.textContent='分析中 '+(i+1)+' / '+symbols.length+'：'+symbol+'｜成功 '+okCount+'｜失敗 '+failCount;
      try{
        const item=await analyzeInstrument(symbol);
        okCount++;
        if(item.totalScore >= (criteria.minTotal==null?55:criteria.minTotal)) rows.push(item);
      }catch(e){ failCount++; console.warn('[Screener]',symbol,e&&e.message?e.message:e); }
      if((i+1)%SCREEN_BATCH_SIZE===0 && i<symbols.length-1) await sleep(SCREEN_BATCH_DELAY);
    }
    renderScreenResults(rows, criteria, { total:symbols.length, ok:okCount, failed:failCount });
    if(_screenScanJob.cancelled){ if(status) status.textContent='掃描已中止'; alert(getScreenMessage('stopped')); }
    else { if(status) status.textContent='篩選完成｜掃描 '+symbols.length+' 檔｜命中 '+rows.length+' 檔'; if(failCount>0) alert(getScreenMessage('partial')); }
    ...
    saveState(state); renderScreenerHistory();
  }finally{ _screenScanJob.running=false; setScreenerRunning(false); }
}
async function generateAIReport(symbol){
  symbol=normalizeSymbol(symbol);
  if(typeof window.navigate==='function') window.navigate('aiReport');
  const title=document.getElementById('aiReportTitle');
  const sub=document.getElementById('aiReportSub');
  const techBox=document.getElementById('aiTechBox');
  const fundBox=document.getElementById('aiFundBox');
  const exp=document.getElementById('aiExplainBox');
  title.textContent='🤖 AI 診斷報告｜'+symbol+' '+(stockNameOf(symbol)||'');
  sub.textContent='系統將以技術面為主，並視標的性質決定是否套用公司型基本面。';
  techBox.innerHTML='資料讀取中…';
  fundBox.innerHTML='資料讀取中…';
  try{
    const item=await analyzeInstrument(symbol);
    techBox.innerHTML=''
      +'<div class="ai-item"><b>現價：</b>'+formatP(item.price)+'</div>'
      +'<div class="ai-item"><b>MA20：</b>'+formatP(item.ma20)+'</div>'
      +'<div class="ai-item"><b>RSI：</b>'+(item.rsi==null?'—':item.rsi.toFixed(2))+'</div>'
      +'<div class="ai-item"><b>技術分數：</b>'+item.techScore+'</div>'
      +'<div class="ai-item"><b>綜合分數：</b>'+item.totalScore+'</div>';
    if(item.fund.notApplicable){
      fundBox.innerHTML=''
        +'<div class="ai-item"><b>基本面：</b>不適用</div>'
        +'<div class="ai-item"><b>EPS：</b>不適用</div>'
        +'<div class="ai-item"><b>體質：</b>不適用</div>';
      exp.textContent='此標的屬於 ETF / ETN / 權證 / 槓反等非公司型商品，EPS、體質與公司型基本面評分不適用；AI 診斷僅保留技術面參考。';
    }else{
      fundBox.innerHTML=''
        +'<div class="ai-item"><b>本益比 PE：</b>'+(item.fund.pe==null?'—':item.fund.pe.toFixed(2))+'</div>'
        +'<div class="ai-item"><b>股價淨值比 PBR：</b>'+(item.fund.pbr==null?'—':item.fund.pbr.toFixed(2))+'</div>'
        +'<div class="ai-item"><b>殖利率：</b>'+(item.fund.dividendYield==null?'—':item.fund.dividendYield+'%')+'</div>'
        +'<div class="ai-item"><b>EPS：</b>'+(item.fund.eps==null?'—':item.fund.eps)+'</div>'
        +'<div class="ai-item"><b>基本面分數：</b>'+(item.fundScore==null?'—':item.fundScore)+'</div>';
      exp.textContent='公司型標的會參考 PE、PBR、殖利率與 EPS 等資料進行簡化評分；請將這些結果視為輔助判讀，不是投資建議。';
    }
  }catch(e){
    techBox.innerHTML='資料取得失敗';
    fundBox.innerHTML='資料取得失敗';
    exp.textContent='產生 AI 診斷時發生錯誤：'+(e&&e.message?e.message:e);
  }
}
function bindScreenerUI(){
  ...
  document.getElementById('btnStopScreener')?.addEventListener('click', function(){ _screenScanJob.cancelled=true; document.getElementById('screenRunStatus').textContent='已收到停止指令，將在目前批次結束後停止。'; });
  ...
  document.querySelectorAll('[data-admin-act]').forEach(function(btn){ btn.addEventListener('click', function(){ handleScreenAdminAction(btn.dataset.adminAct); }); });
  renderScreenerHistory(); setScreenPoolHint(); updateScreenAdminUI(); setScreenerRunning(false);
}

// Patch watchlist fundamentals for non-company instruments.
const _origToggleFundamentals = typeof toggleFundamentals === 'function' ? toggleFundamentals : null;
if(_origToggleFundamentals){
  toggleFundamentals = async function(symbol){
    symbol=normalizeSymbol(symbol);
    if(isNonCompanyInstrument(symbol, stockNameOf(symbol))){
      const rowId='fund-'+symbol;
      const existing=document.getElementById(rowId); if(existing){ existing.remove(); return; }
      const tbody=document.getElementById('watchlistBody'); const tr=tbody?tbody.querySelector('[data-symbol="'+symbol+'"]'):null; if(!tr) return;
      const fRow=document.createElement('tr'); fRow.id=rowId;
      fRow.innerHTML='<td colspan="4" style="padding:10px 16px;background:#0a0f16;border-left:3px solid #388bfd"><div style="font-size:.82rem;color:#dbeafe;line-height:1.8;">'+symbol+' 為 ETF / ETN / 權證 / 槓反等非公司型標的，EPS、體質與公司型基本面指標不適用。</div></td>';
      tr.after(fRow); return;
    }
    return _origToggleFundamentals(symbol);
  };
}

const _origOnPageEnter = typeof window.onPageEnter === 'function' ? window.onPageEnter : null;
window.onPageEnter = function(page){
  if(_origOnPageEnter) _origOnPageEnter(page);
  if(page==='screener') renderScreenerHistory();
};

(function initV391(){
  document.addEventListener('DOMContentLoaded', function(){
    const vEl=document.getElementById('appVersion'); if(vEl) vEl.textContent='v'+APP_VERSION;
    document.title='台股虛擬操盤 v'+APP_VERSION;
    if(!Array.isArray(state.screenHistory)) state.screenHistory=[];
    bindScreenerUI();
  });
})();
